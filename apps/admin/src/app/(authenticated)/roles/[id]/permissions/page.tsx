...
const Page = () => {
    ...
    const totalPermissions = useMemo(
        () =>
            catalog.reduce(
                (count, subject) => count + subject.permissions.length,
                0,
            ),
        [catalog],
    );
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (catalogResponse?.statusCode !== 200) {
                throw new Error(
                    catalogResponse?.message || 'Unable to load catalog',
                );
            }
    ...
    const handleSubjectToggle = useCallback((subject: PermissionGroup) => {
        setSelectedIds(previous => {
            const updated = new Set(previous);
            const permissionIds = subject.permissions.map(
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            if (response?.statusCode !== 200) {
                throw new Error(
                    response?.message || 'Unable to update permissions',
                );
            }
            ...
            setError(
                err?.message || 'Something went wrong while saving changes',
            );
        } finally {
            setIsSaving(false);
        }
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                <Button
                    ...
                    disabled={
                        totalPermissions === 0 ||
                        selectedIds.size === totalPermissions
                    }
                >
                    Select all
                </Button>
            ...
            {isLoading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {filteredCatalog.map(subject => {
                        const selectionState =
                            getSubjectSelectionState(subject);
                ...
                <Card className="border-dashed">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Try clearing the search or selecting a different
                            action filter.
                        </CardDescription>
                    </CardHeader>
                </Card>
